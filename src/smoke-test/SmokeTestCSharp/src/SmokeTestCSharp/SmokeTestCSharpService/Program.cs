    ...
    internal static class Program
    {
        ...
        private static void Main()
        {
            try
            {
                ...
                ActorEventSource.Current.Message("Registering Actor : {0}", "SmokeTestCSharp");

                ActorRuntime.RegisterActorAsync<SmokeTestCSharp>(
                   (context, actorType) => new ActorService(context, actorType)).GetAwaiter().GetResult();

                ActorEventSource.Current.Message("Registered Actor : {0}", "SmokeTestCSharp");
